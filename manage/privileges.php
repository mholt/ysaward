...
<html>
<head>
	<title>Privileges &mdash; <?php echo $WARD ? $WARD->Name." Ward" : SITE_NAME; ?></title>
...
<body>
	...
	<article class="grid-12 group">
		...
		<section class="g-6">
			...
			<form method="POST" action="api/doprivileges.php">
				...
				?>
				<label title="<?php echo($priv->HelpText()); ?>"><input type="checkbox" name="priv[]" value="<?php echo $priv->ID(); ?>"> <?php echo $priv->Privilege(); ?></label><br>
				<?php }	?>
				<br>
			...
			?>
				<tr>
					...
					<td>
						<a href="api/doprivileges.php?action=revoke&id=<?php echo $priv->ID(); ?>&m=<?php echo $mem->ID(); ?>" title="Remove this privilege">Revoke</a>
					</td>
				</tr>
			<?php endwhile; ?>
			</table>
		...
		<section class="g-6">
			...
			?>
				<tr>
					...
					<td>
						<a href="api/doprivileges.php?action=revoke&id=<?php echo $priv->ID(); ?>&c=<?php echo $call->ID(); ?>" title="Remove this privilege">Revoke</a>
					</td>
				</tr>
			<?php endwhile; ?>
			</table>
		...
		<div class="text-center"><a href="#">Back to top</a></a>
...
$(function()
{
	if (window.location.search.indexOf("granted") == 1)
	{
		if (window.location.hash == "#to-member")
			toastr.success("Granted privilege(s) to member");
		else if (window.location.hash == "#to-calling")
			toastr.success("Granted privilege(s) to calling");
	}
	else if (window.location.search.indexOf("revoked") == 1)
	{
		if (window.location.hash == "#by-member")
			toastr.success("Revoked privilege from member");
		else if (window.location.hash == "#by-calling")
			toastr.success("Revoked privilege from calling");
	}
});
</script>
